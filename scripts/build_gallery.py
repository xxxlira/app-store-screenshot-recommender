...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .previews{{display:flex;gap:8px;margin-top:14px;align-items:flex-start}}
  .pv{{flex:1;min-width:0;width:100%;height:auto;max-height:190px;object-fit:contain;border-radius:10px;background:#f0f0f3;border:1px solid var(--line)}}
  ...
  .shot img{{width:100%;height:auto;max-height:74vh;object-fit:contain;display:block;background:#f0f0f3;cursor:zoom-in}}
  ...
  /* 全屏放大查看器 */
  .zoom{{position:fixed;inset:0;z-index:80;display:none;align-items:center;justify-content:center;
  ...
  .zoom-close{{position:absolute;top:18px;right:22px;border:none;background:rgba(255,255,255,.15);
    color:#fff;width:42px;height:42px;border-radius:50%;font-size:26px;line-height:1;cursor:pointer;z-index:2}}
  .zoom-close:hover{{background:rgba(255,255,255,.28)}}
  .zoom-nav{{position:absolute;top:50%;transform:translateY(-50%);border:none;
    background:rgba(255,255,255,.15);color:#fff;width:48px;height:48px;border-radius:50%;
  ...
  .zoom-idx{{position:absolute;bottom:18px;left:50%;transform:translateX(-50%);
    color:#fff;font-size:13px;background:rgba(0,0,0,.45);padding:4px 12px;border-radius:20px}}
...
<div class="zoom" id="zoom">
  <div class="zoom-backdrop" data-zclose></div>
  <button class="zoom-close" data-zclose aria-label="关闭">×</button>
  <button class="zoom-nav zoom-prev" data-zdir="-1" aria-label="上一张">‹</button>
  <img class="zoom-img" id="zoomImg" src="" alt="放大预览">
  <button class="zoom-nav zoom-next" data-zdir="1" aria-label="下一张">›</button>
  <div class="zoom-idx" id="zoomIdx"></div>
</div>
...
let zoomUrls = [];   // 当前产品全部图片 url，供放大查看器左右切换
function openModal(name){{
  ...
  zoomUrls = a.images.map(i => IMG + i.file);
  modalGrid.innerHTML = a.images.map((i, idx) => {{
    const tags = [];
    if (i.category) tags.push(i.category);
    if (i.color) tags.push(i.color);
    return `<div class="shot"><img loading="lazy" data-idx="${{idx}}" src="${{IMG}}${{i.file}}" alt="${{a.name}}">
      <div class="cap">${{tags.map(t=>`<span class="tag">${{t}}</span>`).join('')}}</div></div>`;
  }}).join('');
  modal.classList.add('show');
  document.body.style.overflow = 'hidden';
}}
function closeModal(){{ modal.classList.remove('show'); if(!zoom.classList.contains('show')) document.body.style.overflow=''; }}
modal.addEventListener('click', e => {{ if (e.target.hasAttribute('data-close')) closeModal(); }});

// 详情页图片点击 -> 全屏放大
const zoom = document.getElementById('zoom');
const zoomImg = document.getElementById('zoomImg');
const zoomIdxEl = document.getElementById('zoomIdx');
let zoomIdx = 0;
function showZoom(){{
  if (!zoomUrls.length) return;
  zoomImg.src = zoomUrls[zoomIdx];
  zoomIdxEl.textContent = (zoomIdx + 1) + ' / ' + zoomUrls.length;
}}
function openZoom(idx){{
  if (!zoomUrls.length) return;
  zoomIdx = Math.max(0, Math.min(idx, zoomUrls.length - 1));
  showZoom();
  zoom.classList.add('show');
  document.body.style.overflow = 'hidden';
}}
function stepZoom(d){{ zoomIdx = (zoomIdx + d + zoomUrls.length) % zoomUrls.length; showZoom(); }}
function closeZoom(){{ zoom.classList.remove('show'); if(!modal.classList.contains('show')) document.body.style.overflow=''; }}
modalGrid.addEventListener('click', e => {{
  const img = e.target.closest('.shot img');
  if (img) openZoom(parseInt(img.dataset.idx, 10) || 0);
}});
zoom.addEventListener('click', e => {{
  if (e.target.hasAttribute('data-zclose')) closeZoom();
  else if (e.target.dataset.zdir) stepZoom(parseInt(e.target.dataset.zdir, 10));
}});
document.addEventListener('keydown', e => {{
  if (zoom.classList.contains('show')) {{
